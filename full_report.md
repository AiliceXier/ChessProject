...
系统采用客户端-服务端混合架构，总体架构如图3-1所示。系统自上而下分为四层：Unity客户端层（负责渲染、交互、AI计算）、Cloud Code服务端层（负责在线走法校验与状态同步）、UGS云服务层（提供房间、存储、推送等基础设施）、以及自建ECS服务层（排行榜API与聊天WebSocket）。
...
**结论**：采用混合路由方案，决策流程如图3-2所示。深度1（Easy）和深度3（Medium）使用本地MinMax搜索；深度4（Hard）和深度5（Master）调用Claude API。当深度4/5不可用（如断网）时自动降级到深度3。
...
**状态转换图**：系统状态转换关系如图3-3所示。

| [图3-3] |

在线对战的完整数据流时序关系如图3-4所示。
