...
public class Chess
{
    ...
    public async Task<JoinGameResponse> JoinGame(IExecutionContext context, string lobbyCode)
    {
        ...
        catch (Exception e)
        {
            ...
            var lobbyIdsResponse = await _gameApiClient.Lobby.GetJoinedLobbiesAsync(context, context.AccessToken);
            var joinedLobbyId = lobbyIdsResponse.Data.FirstOrDefault();

            if (string.IsNullOrEmpty(joinedLobbyId))
            {
                throw new InvalidOperationException("Unable to join lobby. Verify lobby code and try again.", e);
            }

            var lobbyResponse = await _gameApiClient.Lobby.GetLobbyAsync(context, context.AccessToken, joinedLobbyId);

            if (lobbyResponse.Data.Players.All(p => p.Id == context.PlayerId))
            {
                throw new InvalidOperationException("No opponent has joined this lobby yet.", e);
            }
    ...
    private async Task<JoinGameResponse> Rejoin(IExecutionContext context, Lobby lobby)
    {
        ...
        if (boardResult?.Value == null || whiteResult?.Value == null)
        {
            throw new InvalidOperationException("Game state is incomplete for this lobby.");
        }

        var chessBoard = ChessBoard.LoadFromFen(boardResult.Value.ToString());
        var opponentId = lobby.Players.Select(p => p.Id).FirstOrDefault(id => id != context.PlayerId);
        if (string.IsNullOrEmpty(opponentId))
        {
            throw new InvalidOperationException("No opponent found in this lobby yet.");
        }

        var whitePlayer = whiteResult.Value.ToString();
